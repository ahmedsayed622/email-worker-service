...
union all
  ...
  FROM BO_DVP_INVOICE_HEADER Ass
       ...
       LEFT JOIN T_BROKER Zs
          ON Ps.ACCOUNT_EXECUTIVE = Zs.BROKER_ID
WHERE 1 = 1 AND  Ass.PROFILE_ID =  11075 and Ass.INVOICE_DATE = TO_NUMBER(:close_date) )
GROUP BY A.INVOICE_TRNX_ID,
         V.INVOICE_TRNX_ID,
         A.OMNI_TRADE,
         A.INVOICE_DATE,
...
-- Trading Report for Profile 1991
-- Bind variable :close_date = YYYYMMDD string
SELECT COALESCE (CAST (A.INVOICE_TRNX_ID AS VARCHAR2 (20)), '0')
            AS INVOICE_TRNX_ID,
         COALESCE (
  ...
  FROM BO_DVP_INVOICE_HEADER Ass
       ...
       LEFT JOIN T_BROKER Zs
          ON Ps.ACCOUNT_EXECUTIVE = Zs.BROKER_ID
WHERE 1 = 1 AND  Ass.PROFILE_ID = 1991 and Ass.INVOICE_DATE = TO_NUMBER(:close_date) )
GROUP BY A.INVOICE_TRNX_ID,
         V.INVOICE_TRNX_ID,
         A.OMNI_TRADE,
         A.INVOICE_DATE,
