...
/**
 * Load company signature HTML with inlined images
 * 
 * @param {string} [languageCode='EN'] - Language code: 'EN' or 'AR'
 * @returns {Promise<string>} Signature HTML with base64-encoded images
 */
export async function getDefaultSignatureHtml(languageCode = 'EN') {
  ...
  try {
    ...
    logger.warn('Default signature file missing; using fallback', { 
      signatureHtmlPath, 
      languageCode, 
      error: err.message 
    });
